...
def derive_fernet_key(password: str, salt: bytes, iterations: int = 200_000) -> bytes:
    ...
    Dùng cho quản lý mật khẩu và ghi chú bảo mật."""
    kdf = PBKDF2HMAC(algorithm=_hashes.SHA256(), length=32, salt=salt, iterations=iterations)
    return base64.urlsafe_b64encode(kdf.derive(password.encode()))

def derive_aesgcm_key(password: str, salt: bytes, n: int = 2**16, r: int = 8, p: int = 1) -> bytes:
    """Dẫn xuất khóa AES-256 từ mật khẩu bằng scrypt (memory-hard — chống
    brute-force bằng GPU/ASIC tốt hơn nhiều so với PBKDF2 thuần).
    Dùng riêng cho tính năng mã hóa file/thư mục (mục 17)."""
...
def file_encrypt_decrypt():
    # V5: khóa AES-256 được dẫn xuất bằng scrypt (memory-hard KDF, chống
    # brute-force bằng GPU/ASIC tốt hơn PBKDF2 thuần), mã hóa bằng AES-256-GCM
    # (AEAD — vừa mã hóa vừa chống chỉnh sửa/giả mạo file .enc). Salt + nonce
    # ngẫu nhiên riêng cho MỖI file. Có thể chọn ẨN LUÔN TÊN FILE GỐC (tên
    # thật được mã hóa cùng nội dung, tên file .enc trên đĩa là ngẫu nhiên).
    #
    # Đây vẫn là mã hóa 2 CHIỀU dựa trên mật khẩu: chỉ ai biết đúng mật khẩu
    # mới giải mã được — kể cả chính bạn cũng cần nhớ mật khẩu, vì tool không
    # lưu nó ở bất kỳ đâu. Tool không gửi dữ liệu đi đâu và không tự lan truyền.
    if not CRYPTO_AVAILABLE:
        print("Cần cài thư viện: pip install cryptography")
        return
    ...
    SALT_LEN = 16
    NONCE_LEN = 12
    SKIP_NAMES = {PASSWORD_DB, NOTES_FILE, CONFIG_FILE, INTEGRITY_DB, DISK_LOG}

    print_header("17. Mã hóa / Giải mã File hoặc Thư mục (AES-256-GCM + scrypt)")
    ...
    def pack_payload(original_name: str, data: bytes) -> bytes:
        name_bytes = original_name.encode("utf-8")
        return len(name_bytes).to_bytes(4, "big") + name_bytes + data

    def unpack_payload(payload: bytes):
        name_len = int.from_bytes(payload[:4], "big")
        name = payload[4:4 + name_len].decode("utf-8")
        data = payload[4 + name_len:]
        return name, data

    def encrypt_single_file(file_path, password, hide_names):
        if file_path.endswith(".enc") or os.path.basename(file_path) in SKIP_NAMES:
            return False, None
        try:
            ...
            key = derive_aesgcm_key(password, salt)
            original_name = os.path.basename(file_path)
            plaintext = pack_payload(original_name, data)
            ciphertext = AESGCM(key).encrypt(nonce, plaintext, MAGIC)
            blob = MAGIC + salt + nonce + ciphertext

            out_dir = os.path.dirname(file_path)
            if hide_names:
                random_name = secrets.token_hex(16) + ".enc"
    ...
    def decrypt_single_file(file_path, password):
        ...
        try:
            ...
            if raw[:len(MAGIC)] != MAGIC:
                print(f"❌ File không đúng định dạng ENCV5: {file_path}")
                return False
            offset = len(MAGIC)
            ...
            try:
                ...
                print(f"❌ Sai mật khẩu (hoặc file bị chỉnh sửa/hỏng): {file_path}")
                return False
            original_name, data = unpack_payload(plaintext)
            out_dir = os.path.dirname(file_path)
            safe_name = os.path.basename(original_name)  # chống path traversal
    ...
    if choice == "1":
        ...
        if os.path.isfile(path):
            ...
            for root, _, files in os.walk(path):
                for file in files:
                    if not file.endswith(".enc") and file not in SKIP_NAMES:
        ...
        print(f"\nSẽ mã hóa {len(files_to_process)} file. Hãy đặt một mật khẩu MẠNH")
        print("và LƯU LẠI CẨN THẬN — nếu quên mật khẩu, không ai (kể cả bạn) có thể")
        print("giải mã lại được, vì tool không lưu mật khẩu ở bất kỳ đâu.\n")
        pw1 = input("Đặt mật khẩu: ").strip()
        pw2 = input("Nhập lại mật khẩu để xác nhận: ").strip()
        if not pw1:
            print("Mật khẩu không được để trống")
            return
        if pw1 != pw2:
            print("❌ Hai lần nhập không khớp. Hủy thao tác.")
            return
        if len(pw1) < 8:
            confirm_weak = input("⚠️  Mật khẩu khá ngắn (<8 ký tự), vẫn tiếp tục? (y/n): ").strip().lower()
            if confirm_weak != "y":
                return

        hide = input("Ẩn luôn tên file gốc trên đĩa (đổi thành tên ngẫu nhiên)? (y/n): ").strip().lower() == "y"

        success_count = 0
        enc_paths = []
        for f_path in files_to_process:
            ok, out_path = encrypt_single_file(f_path, pw1, hide)
            if ok:
                success_count += 1
                enc_paths.append(out_path)

        print(f"\n✅ Đã mã hóa {success_count}/{len(files_to_process)} file.")

        if success_count > 0:
            # Kiểm tra thử: giải mã lại 1 file bằng chính mật khẩu vừa nhập
            # (không ghi ra đĩa), để chắc chắn không xóa nhầm bản gốc.
            try:
                ...
                nonce = raw[offset:offset + NONCE_LEN]; offset += NONCE_LEN
                ciphertext = raw[offset:]
                key = derive_aesgcm_key(pw1, salt)
                AESGCM(key).decrypt(nonce, ciphertext, MAGIC)
                verify_ok = True
            except Exception:
                verify_ok = False
            if not verify_ok:
                print("❌ Kiểm tra thử giải mã thất bại — sẽ KHÔNG xóa file gốc để an toàn.")
                return

            confirm = input("Xác nhận đã lưu mật khẩu an toàn. Xóa các file gốc? (y/n): ").strip().lower()
            if confirm == "y":
                deleted = 0
                for f_path, enc_path in zip(files_to_process, enc_paths):
        ...
        password = input("Nhập mật khẩu để giải mã: ").strip()
        if not password:
            print("Mật khẩu không được để trống")
            return
        ...
        decrypted_enc_paths = []
        for f_path in files_to_process:
            if decrypt_single_file(f_path, password):
                success_count += 1
                decrypted_enc_paths.append(f_path)

        print(f"\n✅ Đã giải mã thành công {success_count}/{len(files_to_process)} file.")

        if success_count > 0:
            confirm = input("Xóa các file .enc đã giải mã thành công? (y/n): ").strip().lower()
            if confirm == "y":
                deleted = 0
                for f_path in decrypted_enc_paths:
                    try:
                        os.remove(f_path)
                        deleted += 1
                    except:
